...
<x-monitor::card :class="trim('flex flex-col p-4 '.($attributes->get('class') ?? ''))">
    <x-monitor::metric :label="$label" :value="number_format(($queued ?? 0) + $processed + $failed + ($released ?? 0))" :size="$size">
        <x-monitor::legend :label="__('monitor::messages.common.failed')" dot="bg-rose-500" :value="number_format($failed)" :color="$failed > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-neutral-900 dark:text-neutral-100'" :size="$size"/>
        @if ($released !== null)
            <x-monitor::legend :label="__('monitor::messages.common.released')" dot="bg-orange-500" :value="number_format($released)" :color="$released > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-neutral-900 dark:text-neutral-100'" :size="$size"/>
        @endif
        <x-monitor::legend :label="__('monitor::messages.common.processed')" dot="bg-neutral-300 dark:bg-neutral-600" :value="number_format($processed)" :size="$size"/>
        @if ($queued !== null)
            <x-monitor::legend :label="__('monitor::messages.common.queued')" dot="bg-amber-500" :value="number_format($queued)" :size="$size"/>
        @endif
    </x-monitor::metric>
    <div class="{{ $size === 'lg' ? 'mt-5' : 'mt-4' }}">
        <x-monitor::bar-chart :since="$since" :until="$until" :height="$height" :series="[
            ['label' => __('monitor::messages.common.processed'), 'dot' => 'bg-neutral-300 dark:bg-neutral-600', 'data' => $processedBuckets],
            ...($queuedBuckets !== null ? [['label' => __('monitor::messages.common.queued'), 'dot' => 'bg-amber-500', 'data' => $queuedBuckets]] : []),
            ...($releasedBuckets !== null ? [['label' => __('monitor::messages.common.released'), 'dot' => 'bg-orange-500', 'data' => $releasedBuckets]] : []),
