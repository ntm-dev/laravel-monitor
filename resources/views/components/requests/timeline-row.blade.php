{{-- TimelineRow: one waterfall row — request root, phase header or event.
     ...
     scope set up by timeline.blade.php (selectedId/data). --}}
<div class="group grid h-7 grid-cols-[15rem_1fr] items-stretch"
     :class="selectedId === '{{ $entry->id }}' ? 'bg-blue-50/60 dark:bg-blue-500/5' : ''"
     @click="selectedId = (selectedId === '{{ $entry->id }}' ? null : '{{ $entry->id }}')">

    {{-- Pinned label column --}}
    <div class="sticky left-0 z-20 flex min-w-0 cursor-pointer items-center gap-1.5 bg-white pr-3 group-hover:bg-neutral-50 dark:bg-neutral-900 dark:group-hover:bg-neutral-800/60
        {{ $kind === 'event' ? 'pl-7' : 'pl-3' }}">
        @if ($kind === 'root')
    ...
    <div class="relative cursor-pointer group-hover:bg-neutral-50/60 dark:group-hover:bg-neutral-800/30">
        ...
        @else
            <div class="absolute top-1/2 h-3.5 -translate-y-1/2 rounded-sm {{ $color }}"
                 style="left: {{ $left }}%; width: {{ $width }}%; min-width: 3px"
                 :class="selectedId === '{{ $entry->id }}' ? 'ring-1 ring-blue-500 ring-offset-1 dark:ring-offset-neutral-900' : ''"></div>
