...
<x-monitor::card :class="trim('flex flex-col p-4 '.($attributes->get('class') ?? ''))" x-data="{ hidden: {} }">
    <x-monitor::metric :label="__('monitor::messages.nav.requests')" :value="number_format($count)">
        ...
        @if ($failed !== null)
            <x-monitor::legend :label="__('monitor::messages.common.failed')" dot="bg-violet-500" :value="number_format($failed)" :color="$failed > 0 ? 'text-violet-600 dark:text-violet-400' : 'text-neutral-900 dark:text-neutral-100'" series-key="failed"/>
        @endif
    </x-monitor::metric>
    <div class="mt-5">
