...
<x-monitor::card :class="trim('flex flex-col p-4 '.($attributes->get('class') ?? ''))">
    <x-monitor::metric label="Requests" :value="number_format($count)">
        <x-monitor::legend label="1/2/3XX" dot="bg-neutral-300" :value="number_format($ok)"/>
        <x-monitor::legend label="4XX" dot="bg-amber-500" :value="number_format($client)" :color="$client > 0 ? 'text-amber-600' : 'text-neutral-900'"/>
        <x-monitor::legend label="5XX" dot="bg-rose-500" :value="number_format($server)" :color="$server > 0 ? 'text-rose-600' : 'text-neutral-900'"/>
    </x-monitor::metric>
    <div class="mt-5">
        <x-monitor::bar-chart :since="$since" :until="$until" :height="$height" :series="[
            ['label' => '1/2/3XX', 'dot' => 'bg-neutral-300', 'data' => $okBuckets],
