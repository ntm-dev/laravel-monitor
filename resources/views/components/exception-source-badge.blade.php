{{-- Which request/job/command an exception occurrence happened during, linking to that run's timeline when known. --}}
...
@if ($type && $colors && $url)
    <a href="{{ $url }}" class="flex items-center gap-1.5 hover:underline">
        ...
        <span class="truncate font-mono text-xs text-neutral-600 dark:text-neutral-300" title="{{ $label }}">{{ $label ?? '—' }}</span>
    </a>
@else
