                    ...
                    <tbody class="divide-y divide-neutral-100">
                        @foreach ($jobs as $job)
                            <tr class="group cursor-pointer hover:bg-neutral-50"
                                ...
                                <td class="max-w-[14rem] truncate py-2 pr-2 font-mono text-xs text-neutral-700" title="{{ $job->key }}">{{ class_basename($job->key) }}</td>
                                <td class="py-2 text-right font-mono text-xs text-neutral-600">{{ number_format($job->queued) }}</td>
                                <td class="py-2 text-right font-mono text-xs text-emerald-600">{{ number_format($job->processed) }}</td>
                                <td class="py-2 text-right font-mono text-xs {{ $job->failed > 0 ? 'text-rose-600' : 'text-neutral-300' }}">{{ number_format($job->failed) }}</td>
                                <td class="py-2 text-right font-mono text-xs text-neutral-600">{{ $fmt($job->avg_duration) }}</td>
