    ...
    @else
        <x-monitor::card class="flex flex-col gap-6 p-4 md:flex-row">
            <div class="md:w-1/2">
                ...
                <dl class="flex flex-col gap-3">
                    <div class="flex max-w-full items-baseline gap-2">
                        ...
                        <dd class="max-w-[60%] truncate font-mono text-xs text-neutral-900 dark:text-white" data-tooltip="{{ $entry->payload['notification'] ?? $entry->key }}">{{ $entry->payload['notification'] ?? $entry->key }}</dd>
                    </div>
                    <div class="flex max-w-full items-baseline gap-2">
                    ...
                    <div class="flex max-w-full items-baseline gap-2">
                        ...
                        <div class="relative -bottom-px min-w-6 grow border-b-2 border-dotted border-neutral-300 dark:border-white/20"></div>
                        <dd class="max-w-[60%] truncate font-mono text-xs text-neutral-900 dark:text-white" data-tooltip="{{ $entry->payload['notifiable'] ?? '' }}">{{ $entry->payload['notifiable'] ?? '—' }}</dd>
                    </div>
                    <div class="flex max-w-full items-baseline gap-2">
            ...
            <div class="flex flex-col justify-between gap-4 rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-700 dark:bg-neutral-900 md:w-1/2">
                ...
                @if ($mail !== null)
                    <a href="{{ route('monitor.mail.sends.show', ['hash' => \LaravelMonitor\Support\KeyHash::for($mail->key), 'id' => \LaravelMonitor\Support\EntryId::encode($mail->id)] + $range) }}"
