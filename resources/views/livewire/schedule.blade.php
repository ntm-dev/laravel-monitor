@php
    ...
    $columns = [
        'key' => ['label' => 'Task', 'align' => 'left'],
        'finished' => ['label' => 'Finished', 'align' => 'right'],
        'skipped' => ['label' => 'Skipped', 'align' => 'right'],
...
<div wire:poll.{{ $refresh }}s>
    <x-monitor::section :icon="Icons::SCHEDULE" title="Scheduled Tasks">
        <x-slot:actions>
        ...
        <div class="grid grid-cols-1 gap-1.5 lg:grid-cols-2"
            ...
            <x-monitor::card class="flex flex-col p-4">
                <x-monitor::metric label="Scheduled Tasks" :value="number_format($finished + $failed + $skipped)">
                    ...
                    <x-monitor::legend label="Skipped" dot="bg-neutral-300 dark:bg-neutral-600" :value="number_format($skipped)"/>
                    <x-monitor::legend label="Processed" dot="bg-emerald-500" :value="number_format($finished)"/>
                </x-monitor::metric>
                <div class="mt-5">
                    <x-monitor::bar-chart :since="$since" :until="$until" height="h-[167px]" :series="[
                        ['label' => 'Processed', 'dot' => 'bg-emerald-500', 'data' => $finishedBuckets],
                        ['label' => 'Skipped', 'dot' => 'bg-neutral-300 dark:bg-neutral-600', 'data' => $skippedBuckets],
        ...
        <div class="mt-4 flex items-center justify-between gap-2 px-1 pb-3">
            <h3 class="font-semibold text-neutral-900 dark:text-neutral-100">{{ number_format($totalTasks) }} {{ $totalTasks === 1 ? 'Task' : 'Tasks' }}</h3>
            <div class="relative">
            ...
            <x-monitor::card class="p-4">
                ...
                @if ($lastPage > 1)
                    <div class="mt-3 flex items-center justify-between border-t border-neutral-100 dark:border-neutral-800 pt-3 font-mono text-xs text-neutral-500 dark:text-neutral-400">
                        <span>Showing {{ $from + 1 }}–{{ min($from + $perPage, $totalTasks) }} of {{ number_format($totalTasks) }}</span>
