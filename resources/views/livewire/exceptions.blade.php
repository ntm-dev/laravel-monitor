...
<div wire:poll.{{ $refresh }}s>
    <x-monitor::section :icon="Icons::EXCEPTIONS" title="Exceptions">
        ...
        <div class="mt-4 flex flex-wrap items-center justify-between gap-3 px-1 pb-3">
            <h3 class="font-semibold text-neutral-900">{{ number_format($totalGroups) }} {{ $totalGroups === 1 ? 'Exception' : 'Exceptions' }}</h3>
            <div class="flex items-center gap-2">
                <div class="flex h-8 items-center gap-0.5 rounded-lg border border-neutral-200 bg-white p-0.5 shadow-sm">
                    @foreach ($filters as $value => $label)
                        <button type="button" wire:click="setStatus('{{ $value }}')"
                                @class([
                                    ...
                                    'text-neutral-500 hover:text-neutral-900' => $status !== $value,
                                ])>{{ $label }}</button>
                    @endforeach
                </div>
                <div class="relative">
                                ...
                                <tr class="group cursor-pointer {{ $group->unhandled > 0 ? 'hover:bg-rose-50/50' : 'hover:bg-neutral-50' }}"
                                    ...
                                    <td class="max-w-[26rem] py-2.5 pr-3">
                                        ...
                                        @elseif (filled($group->file))
                                            <p class="mt-0.5 truncate font-mono text-[11px] text-neutral-400">{{ $group->file }}:{{ $group->line }}</p>
                                        @endif
                                    </td>
                                    <td class="whitespace-nowrap py-2.5 text-right font-mono text-xs font-medium text-neutral-700">{{ number_format($group->count) }}</td>
