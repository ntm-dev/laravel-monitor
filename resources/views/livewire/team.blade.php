...
<div wire:poll.{{ $refresh }}s>
    <x-monitor::section :icon="Icons::TEAM" title="{{ __('monitor::messages.nav.team') }}">
        <div class="mt-4 flex items-center gap-2 px-1 pb-3">
            <h3 class="font-semibold text-neutral-900 dark:text-neutral-100">{{ $t('my_account') }}</h3>
        </div>

        <x-monitor::card class="p-4">
            <p class="font-mono text-xs uppercase tracking-tight text-neutral-500 dark:text-neutral-400">{{ $t('email_address') }}</p>
            @if ($emailChangeRequested)
                <p class="mt-2 text-sm text-emerald-600 dark:text-emerald-400">{{ $t('email_change_sent') }}</p>
            @endif
            @if (! $editingEmail)
                <div class="mt-3 flex items-center gap-1.5">
                    <span class="min-w-0 flex-1 truncate text-sm text-neutral-700 dark:text-neutral-200">{{ $actor->email }}</span>
                    <button type="button" wire:click="startEditingEmail" title="{{ $t('edit_email') }}"
            ...
            @else
                <form x-on:submit.prevent="$wire.requestEmailChange($refs.newEmail.value, $refs.currentPassword.value)" class="mt-3 flex flex-wrap items-end gap-2" x-data>
                    ...
                    <button type="submit" class="h-8 rounded-md bg-blue-600 px-3 text-sm font-medium text-white hover:bg-blue-500">{{ $t('verify') }}</button>
        ...
        <x-monitor::card class="mt-4 p-4" x-data="{ open: false }" x-on:password-changed.window="open = false">
            <p class="font-mono text-xs uppercase tracking-tight text-neutral-500 dark:text-neutral-400">{{ $t('password') }}</p>
            @if ($passwordChanged)
                <p class="mt-2 text-sm text-emerald-600 dark:text-emerald-400">{{ $t('password_changed') }}</p>
            @endif
            <button type="button" x-on:click="open = true"
                    class="mt-3 h-8 rounded-md border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 text-sm font-medium text-neutral-700 dark:text-neutral-200 shadow-sm hover:bg-neutral-50 dark:hover:bg-neutral-800/50">{{ $t('change_password') }}</button>
        ...
        <x-monitor::card class="mt-4 p-4">
            <p class="font-mono text-xs uppercase tracking-tight text-neutral-500 dark:text-neutral-400">{{ $t('two_factor') }}</p>
            @if (! \LaravelMonitor\Support\OptionalAuthMethod::totpAvailable())
                <p class="mt-2 text-sm text-neutral-400 dark:text-neutral-500">{{ $t('totp_install_hint', ['packages' => 'pragmarx/google2fa bacon/bacon-qr-code']) }}</p>
            @else
                <div class="mt-3"
                     x-data="{ open: false, totpMode: '{{ $actor->hasTotpEnabled() ? 'enabled' : 'disabled' }}' }"
                     x-on:totp-enabled.window="open = false"
                     x-on:totp-disabled.window="open = false; totpMode = 'disabled'">
                    <select
                        x-model="totpMode"
                        x-on:change="open = true; if (totpMode === 'enabled') { $wire.startEnrollingTotp(); }"
                        ...
                        <option value="disabled">{{ $t('disabled_status') }}</option>
                        <option value="enabled">{{ $t('enabled_status') }}</option>
                    </select>

                    <div x-show="open" x-cloak
                         x-on:keydown.escape.window="open = false; totpMode = '{{ $actor->hasTotpEnabled() ? 'enabled' : 'disabled' }}'; $wire.cancelEnrollingTotp()"
                         class="{{ $popupOverlay }}">
                        <div x-on:click.outside="open = false; totpMode = '{{ $actor->hasTotpEnabled() ? 'enabled' : 'disabled' }}'; $wire.cancelEnrollingTotp()" class="{{ $popupCard }}">
                            <template x-if="totpMode === 'enabled'">
                                <div>
                                    ...
                                    @else
                                        ...
                                        <form x-on:submit.prevent="$wire.confirmTotp($refs.totpCode.value)" class="mt-3 space-y-3">
                                            ...
                                            <div class="flex justify-end gap-2 pt-1">
                                                <button type="button"
                                                        x-on:click="open = false; totpMode = '{{ $actor->hasTotpEnabled() ? 'enabled' : 'disabled' }}'; $wire.cancelEnrollingTotp()"
                            ...
                            <template x-if="totpMode === 'disabled'">
                                <div>
        ...
        <x-monitor::card class="mt-4 p-4">
            <p class="font-mono text-xs uppercase tracking-tight text-neutral-500 dark:text-neutral-400">{{ $t('passkeys') }}</p>
            @if (! \LaravelMonitor\Support\OptionalAuthMethod::passkeysAvailable())
                <p class="mt-2 text-sm text-neutral-400 dark:text-neutral-500">{{ $t('passkeys_install_hint', ['package' => 'web-auth/webauthn-lib']) }}</p>
            @else
                ...
                <div x-data="{ open: false }">
                    <button type="button" x-on:click="open = true" class="mt-3 h-8 rounded-md bg-blue-600 px-3 text-sm font-medium text-white hover:bg-blue-500">{{ $t('add_passkey') }}</button>
        ...
        <div class="mt-6 flex items-center justify-between gap-2 px-1 pb-3">
            <h3 class="font-semibold text-neutral-900 dark:text-neutral-100">{{ number_format($members->count()) }} {{ $members->count() === 1 ? $t('member') : $t('members') }}</h3>
            @if ($actor->canManageTeam())
                <div x-data="{ open: false }" x-on:member-invited.window="open = false">
                    <button type="button" x-on:click="open = true" class="h-8 rounded-md bg-blue-600 px-3 text-sm font-medium text-white hover:bg-blue-500">{{ $t('invite') }}</button>
