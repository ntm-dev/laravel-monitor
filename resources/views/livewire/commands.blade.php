@php
    ...
    $columns = [
        'key' => ['label' => 'Command', 'align' => 'left'],
        'success' => ['label' => 'Success', 'align' => 'right'],
...
<div wire:poll.{{ $refresh }}s>
    <x-monitor::section :icon="Icons::COMMANDS" title="Commands">
        <x-slot:actions>
        ...
        <div class="grid grid-cols-1 gap-1.5 lg:grid-cols-2"
            ...
            <x-monitor::duration-chart-card label="Duration" :duration="$duration" :since="$since" :until="$until" :threshold="$threshold" height="h-[167px]"/>
        </div>

        {{-- Command table --}}
        <div class="mt-4 flex items-center justify-between gap-2 px-1 pb-3">
            <h3 class="font-semibold text-neutral-900 dark:text-neutral-100">{{ number_format($totalCommands) }} {{ $totalCommands === 1 ? 'Command' : 'Commands' }}</h3>
            <div class="relative">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search commands…"
            ...
            <x-monitor::card class="p-4">
                ...
                @if ($lastPage > 1)
                    <div class="mt-3 flex items-center justify-between border-t border-neutral-100 dark:border-neutral-800 pt-3 font-mono text-xs text-neutral-500 dark:text-neutral-400">
                        <span>Showing {{ $from + 1 }}–{{ min($from + $perPage, $totalCommands) }} of {{ number_format($totalCommands) }}</span>
